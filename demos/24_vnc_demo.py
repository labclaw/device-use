#!/usr/bin/env python3
"""AI operates TopSpin via VNC — no VM-side permissions needed.
...
Usage:
    # 1. Start VM:   lume run topspin-vm
...
def get_vnc_info() -> tuple[str, int, str]:
    ...
    for line in result.stdout.strip().split("\n"):
        if "topspin-vm" in line and "running" in line:
            m = re.search(r"vnc://:([\w-]+)@([\d.]+):(\d+)", line)
            if m:
                password, host, port = m.group(1), m.group(2), int(m.group(3))
                return f"{host}::{port}", password
    raise RuntimeError("topspin-vm not running or VNC not found")


def vnc_connect(vnc_addr: str, password: str):
    """Create a VNC connection."""
    return vncapi.connect(vnc_addr, password=password)


def vnc_screenshot(vnc_addr: str, password: str, path: str = "/tmp/vnc_frame.png") -> Image.Image:
    """Take a screenshot via VNC, return PIL Image."""
    c = vnc_connect(vnc_addr, password)
    c.captureScreen(path)
    c.disconnect()
    return Image.open(path)


def vnc_click(vnc_addr: str, password: str, x: int, y: int):
...
def vnc_type_text(vnc_addr: str, password: str, text: str):
...
def vnc_key(vnc_addr: str, password: str, key: str):
...
def vnc_release_modifiers(vnc_addr: str, password: str):
...
def vnc_type_command(vnc_addr: str, password: str, cmd: str):
    ...
    # Type the command
    vnc_type_text(vnc_addr, password, cmd)
    time.sleep(0.2)

    # Press Return to execute
    vnc_key(vnc_addr, password, "return")
...
def screenshot_and_push(vnc_addr: str, password: str) -> tuple[Image.Image, bytes]:
    """Take screenshot, push JPEG to stream, return (image, png_bytes)."""
    img = vnc_screenshot(vnc_addr, password)
...
def step_open_topspin(vnc_addr: str, password: str, ai: OpenAI) -> bool:
    ...
    if coords:
        vnc_click(vnc_addr, password, coords[0], coords[1])
        time.sleep(5)
        img, png = screenshot_and_push(vnc_addr, password)
        b64 = base64.b64encode(png).decode()
...
def step_load_dataset(vnc_addr: str, password: str, ai: OpenAI) -> bool:
    """Load NMR dataset using 're' command."""
    push_log("▶ Load Dataset", status="operating")

    vnc_type_command(vnc_addr, password, f"re {DATASET}")
    time.sleep(5)
...
def step_run_command(vnc_addr: str, password: str, ai: OpenAI,
    ...
    vnc_type_command(vnc_addr, password, cmd)
    time.sleep(3)
...
def step_verify_result(vnc_addr: str, password: str, ai: OpenAI) -> bool:
...
def main() -> int:
    ...
    # Get VNC info from lume
    print("  Getting VNC info...")
    try:
        vnc_addr, password = get_vnc_info()
        print(f"  {G}✓ VNC: {vnc_addr}{RST}")
    ...
    vnc_release_modifiers(vnc_addr, password)
    time.sleep(0.5)
    ...
    for _dismiss in range(5):
        ...
        if "license" in desc.lower() or "accept" in desc.lower():
            ...
            if coords:
                vnc_click(vnc_addr, password, coords[0], coords[1])
                time.sleep(3)
                screenshot_and_push(vnc_addr, password)
                continue
        vnc_key(vnc_addr, password, "return")
        time.sleep(1)
        screenshot_and_push(vnc_addr, password)
    ...
    # Step 1: Open TopSpin
    ok = step_open_topspin(vnc_addr, password, ai)
    ...
    # Step 2: Load Dataset
    ok = step_load_dataset(vnc_addr, password, ai)
    ...
    ok = step_verify_result(vnc_addr, password, ai)
    ...
    vnc_screenshot(vnc_addr, password, final_path)
